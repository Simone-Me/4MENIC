...
function isCinemaConnectedToAdmin($cinemaName)
{
    ...
    // Prepare and bind
    $stmt = $conn->prepare("SELECT a.idAdmin, a.idUser, a.idCinema, c.nomCine AS cinemaName
        FROM admin a JOIN cinema c ON a.idCinema = c.idCinema
        WHERE c.nomCine = ?
    ");
            ...
            <tbody>
                <tr>
                    ...
                    <td class="colomn">
                        ...
                        ?>
                            <a style="color: lightgreen;" href="/4MENIC/PHP/bookRoom.php?nomCine=<?= $single["nomCine"] ?>"> <i class="bi bi-house-add"></i></a>
                        <?php } else { ?>
                            NO <?php } ?>
                    </td>
                </tr>
